...
$form = ActiveForm::begin(['action' => '#!',
            ...
            'options' => ['onsubmit' => 'EditarCampoFormulario(' . $model_formulario->idseccion . ', "' . $nombre_seccion . '"); return false;', 'id' => 'form_EditarCampoFormulario']]);

Modal::begin([
    'header' => '<h4>Editar Campo</h4>',
    'id' => 'modal_formulario_campo',
    'size' => 'modal-lg',
]);

echo $form->field($model_formulario, 'orden')->staticInput();
...
echo $form->field($model_formulario, 'new_orden')->widget(Select2::classname(), [
    'data' => $posiciones,
    'options' => ['placeholder' => 'Nuevo Orden'],
    'pluginOptions' => [
        'allowClear' => true
    ],
])->label('Nuevo Orden');

$model_formulario->req_bol = $model_formulario->requerido == 'SI' ? true : false;
...
echo $form->field($model_formulario, 'tipo_entrada')->radioList($list, ['inline' => true])->label("Valores Aceptados");

echo $form->field($model_formulario, 'id_campo')->hiddenInput()->label(false);
...
<div class="modal-footer">
    <?= Html::submitButton('<span clas="fa fa-save"></span> Guardar Cambios', ['class' => 'btn btn-primary btn-block']) ?>
</div>
